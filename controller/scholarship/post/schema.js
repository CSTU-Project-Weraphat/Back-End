...
const schema = (req, res, next) => {
    const validateBody = Joi.object().keys({
        scholarship_name: Joi.string().min(3).max(500).required(),
        ...
        start_date:Joi.date().utc().required(),
        end_date:Joi.date().utc().required(),
        scholarship_type_id:Joi.number().integer().strict().required(),
        color_tag:Joi.string().required(),
        scholarship_condition: Joi.string().min(3).max(500).required(),
        scholarship_qualification: Joi.string().min(3).max(1000).required(),
    })
